...
int altaFiguritas(eFigurita arrayFiguritas[], int tamFiguritas, int *id, eEquipo arrayEquipos[], int tamEquipos, eClub arrayClubs[], int tamClubs)
{
	...
	if (arrayFiguritas != NULL && tamFiguritas > 0 && id != NULL && arrayEquipos != NULL && tamEquipos > 0)
	{
		...
		if (indiceLibre == -1)
		{
			printf("\nNo hay espacio para m�s figuritas.\n");
		}
		else
		{
			auxFigurita.id = *id;
			...
			pedirEntero(&auxFigurita.anioIngresoSeleccion, "\nIngrese el anio de ingreso del jugador (entre 1930 y 2022): ",
					"Error. El anio ingresado no es valido.\n", 1930, 2022);
...
void mostrarUnaFigurita(eFigurita unaFigurita, eEquipo arrayEquipos[], int tamEquipos, eClub clubs[], int tamClubs)
{
	...
	printf("\n| %-4d | %-23s | %-14s | %-10s | %-28s | %-8.2f | %-8.2f | %-14d | %2.d/%2.d/%-13d | %-14s | ", unaFigurita.id, unaFigurita.nombreJugador,
			unaFigurita.posicion, descripcion, directorTecnico, unaFigurita.altura, unaFigurita.peso, unaFigurita.anioIngresoSeleccion,
			unaFigurita.fechaNacimiento.dia, unaFigurita.fechaNacimiento.mes, unaFigurita.fechaNacimiento.anio, nombreClub);
...
int listarFiguritasRepetidas_2(eFigurita arrayRepetidas[], int tamRepetidas, eEquipo arrayEquipos[], int tamEquipos, eClub arrayClubs[], int tamClub)
{
	...
	if (arrayRepetidas != NULL && tamRepetidas > 0)
	{
		...
		for (int i = 0; i < tamRepetidas; i++)
		{
			if (arrayRepetidas[i].estado == OCUPADO)
			{
				mostrarUnaFigurita(arrayRepetidas[i], arrayEquipos, tamEquipos, arrayClubs, tamClub);
				flag = 1;
			}
		}
		printf("\n-----------------------------------------------------------");
...
int hardcodeoDeFiguritas(eFigurita arrayFiguritas[], int tamFiguritas, int *id, int *flag)
{
	...
	int equipoCargado[55] =
	{ 1001, 1003, 1000, 1000, 1003, 1004, 1002, 1001, 1000, 1001, 1002, 1001, 1004, 1001, 1003, 1000, 1003, 1004, 1003, 1002, 1002, 1000, 1000, 1000, 1002,
			1004, 1003, 1003, 1000, 1001, 1000, 1002, 1001, 1003, 1004, 1002, 1001, 1004, 1003, 1002, 1004, 1004, 1000, 1004, 1001, 1003, 1000, 1002, 1002,
			1003, 1004, 1001, 1002, 1004, 1001 };

	float alturaCargada[55] =
	{ 2.51, 2.44, 1.14, 1.59, 1.59, 1.2, 2.81, 2.64, 2.02, 1.22, 2.51, 2.44, 1.14, 1.59, 1.59, 1.2, 2.81, 2.64, 2.02, 1.22, 2.51, 2.44, 1.14, 1.59, 1.59, 1.2,
			2.81, 2.64, 2.02, 1.22, 2.51, 2.44, 1.14, 1.59, 1.59, 1.2, 2.81, 2.64, 2.02, 1.22, 2.51, 2.44, 1.14, 1.59, 1.59, 1.2, 2.81, 2.64, 2.02, 1.22, 2.51,
			2.44, 1.14, 1.59, 1.59 };

	float pesoCargado[55] =
	{ 87.05, 73.73, 93.16, 51.57, 88.56, 101.03, 93.67, 81.89, 96.82, 42.8, 87.05, 73.73, 93.16, 51.57, 88.56, 101.03, 93.67, 81.89, 96.82, 42.8, 87.05, 73.73,
			93.16, 51.57, 88.56, 101.03, 93.67, 81.89, 96.82, 42.8, 87.05, 73.73, 93.16, 51.57, 88.56, 101.03, 93.67, 81.89, 96.82, 42.8, 87.05, 73.73, 93.16,
			51.57, 88.56, 101.03, 93.67, 81.89, 96.82, 42.8, 87.05, 73.73, 93.16, 51.57, 88.56 };

	int anioIngresoCargado[55] =
	{ 2017, 2015, 2020, 2000, 2000, 2003, 2018, 2022, 2019, 2020, 2006, 2008, 2016, 2013, 2000, 2002, 2015, 2022, 2019, 2018, 2017, 2015, 2020, 2000, 2000,
			2003, 2018, 2022, 2019, 2020, 2006, 2008, 2016, 2013, 2000, 2002, 2015, 2022, 2019, 2018, 2017, 2015, 2020, 2000, 2000, 2003, 2018, 2022, 2019,
			2020, 2006, 2008, 2016, 2013, 2000 };

	int fechaDiaCargado[55] =
	{ 03, 05, 25, 26, 21, 12, 7, 3, 9, 12, 03, 05, 25, 26, 21, 12, 7, 3, 9, 12, 03, 05, 25, 26, 21, 12, 7, 3, 9, 12, 03, 05, 25, 26, 21, 12, 7, 3, 9, 12, 03,
			05, 25, 26, 21, 12, 7, 3, 9, 12, 03, 05, 25, 26, 21 };

	int fechaMesCargado[55] =
	{ 02, 03, 04, 05, 06, 07, 03, 10, 11, 02, 03, 04, 05, 06, 07, 03, 10, 11, 05, 12, 02, 03, 04, 05, 06, 07, 03, 10, 11, 02, 03, 04, 05, 06, 07, 03, 10, 11,
			05, 12, 02, 03, 04, 05, 06, 07, 03, 10, 11, 02, 03, 04, 05, 06, 07 };

	int fechaAnioCargado[55] =
	{ 2000, 2003, 1993, 2000, 2003, 1994, 1997, 1999, 1991, 2001, 2000, 2003, 1993, 2000, 2003, 1994, 1997, 1999, 1991, 2001, 2000, 2003, 1993, 2000, 2003,
			1994, 1997, 1999, 1991, 2001, 2000, 2003, 1993, 2000, 2003, 1994, 1997, 1999, 1991, 2001, 2000, 2003, 1993, 2000, 2003, 1994, 1997, 1999, 1991,
			2001, 2000, 2003, 1993, 2000, 2003 };

	int club[55] =
	{ 104, 100, 107, 100, 102, 107, 102, 105, 109, 108, 104, 100, 107, 102, 108, 101, 109, 106, 108, 101, 100, 100, 102, 105, 104, 106, 105, 107, 100, 105, 100,
			100, 108, 104, 107, 106, 100, 102, 107, 108, 108, 109, 107, 103, 102, 106, 103, 104, 102, 105, 103, 105, 100, 106, 106 };

	int estadoCargado[55] =
	{ OCUPADO, OCUPADO, OCUPADO, OCUPADO, OCUPADO, OCUPADO, OCUPADO, OCUPADO, OCUPADO, OCUPADO, OCUPADO, OCUPADO, OCUPADO, OCUPADO, OCUPADO, OCUPADO, OCUPADO,
	OCUPADO, OCUPADO, OCUPADO, OCUPADO, OCUPADO, OCUPADO, OCUPADO, OCUPADO, OCUPADO, OCUPADO, OCUPADO, OCUPADO, OCUPADO, OCUPADO, OCUPADO, OCUPADO,
	OCUPADO, OCUPADO, OCUPADO, OCUPADO, OCUPADO, OCUPADO, OCUPADO, OCUPADO, OCUPADO, OCUPADO, OCUPADO, OCUPADO, OCUPADO, OCUPADO, OCUPADO, OCUPADO,
	OCUPADO, OCUPADO, OCUPADO, OCUPADO, OCUPADO, OCUPADO };

	int estadoDoradaCargado[55] =
	{ NO, NO, NO, NO, NO, NO, NO, NO, NO, NO, SI, SI, SI, SI, SI, SI, SI, SI, SI, SI, NO, NO, NO, NO, NO, NO, NO, NO, NO, NO, SI, SI, SI, SI, SI, SI, SI, SI,
	SI, SI, NO, NO, NO, NO, NO, NO, NO, NO, NO, NO, SI, SI, SI, SI, SI };
		...
		int *precioAlbumCompleto, eClub clubs[], int tamClubs)
{
	...
	if (arrayFiguritas != NULL && tamFiguritas > 0)
	{
		...
		*precioAcumulado += precio;
		...
		printf("\n******************************************************************************************");
		printf("\n                        PAQUETE DE FIGURITAS VENDIDO CON �XITO                            ");
		printf("\n******************************************************************************************");
		...
		for (int i = 0; i < contadorFiguritas; i++)
		{
			...
			if (arrayFiguritas[indiceRandom].estado == OCUPADO)
			{
				if (hay10Sobres == 10 && arrayFiguritas[indiceRandom].esDorada == SI)
				{
					...
					pegarFigurita(album, indiceLibreAlbum, arrayFiguritas, indiceRandom);
				...
				if (arrayFiguritas[indiceRandom].esDorada == NO)
				{
					...
					if (buscarFiguritasRepetidasPorID_Album(album, arrayFiguritas[indiceRandom].id) == 0)
					{
						if (indiceLibreAlbum == -1)
						{
							...
							calcularPrecioTotal(album, &flagSobres, cantidadSobresTotal, contadorSobres, precioAlbumCompleto, precioAcumulado);
						}
						else
						{
							pegarFigurita(album, indiceLibreAlbum, arrayFiguritas, indiceRandom);
		...
		int tamClubs)
{
	...
	printf(
			"\n=========================== listado de figuritas de %s pegadas ==============================================================================================================",
			criterio);
	encabezadoFigurita_1();
	for (int i = 0; i < tamFiguritas; i++)
	{
...
int listarFiguritasPegadasPorEquipo(eFigurita arrayFiguritas[], int tamFiguritas, eEquipo equipos[], int tamEquipos, eClub clubs[], int tamClubs)
{
	...
	if (arrayFiguritas != NULL && tamFiguritas > 0)
	{
		...
		idEquipo = menuListarPegadasPorEquipo(equipos);

		if (paisSeleccionado != NULL)
		{
			for (int i = 0; i < tamFiguritas; i++)
			{
				if (arrayFiguritas[i].estadoAlbum == PEGADO && arrayFiguritas[i].idEquipo == idEquipo)
				{
					mostrarUnaFigurita(arrayFiguritas[i], equipos, tamEquipos, clubs, tamClubs);
					todoOk = 0;
				}
		...
		int *flag, eClub clubs[], int tamClubs)
{
	...
	if (arrayFiguritas != NULL && equipos > 0)
	{
		if (*flag == 1)
		{
			...
			{
				listarFiguritasRepetidas_2(arrayRepetidas, tamRepetidas, equipos, tamEquipos, clubs, tamClubs);
...
{
	int todoOk = -1;
	if(arrayFiguritas != NULL && tamFiguritas > 0 && arrayClubs != NULL && contadores != NULL)
	{
		for (int i = 0; i < tamClubs; i++)
		{
			for (int j = 0; j < tamFiguritas; j++)
			{
				if (arrayFiguritas[j].idClub == arrayClubs[i].id && arrayFiguritas[j].estadoAlbum == PEGADO)
				{
					contadores[i]++;
				}
			}
...
{
	int mayorCantClubs;
	if(contadores != NULL && tamClubs > 0)
	{
		for (int i = 0; i < tamClubs; i++)
		{
			if (i == 0 || mayorCantClubs < contadores[i])
			{
				mayorCantClubs = contadores[i];
			}
		}
	}
	return mayorCantClubs;
}
...
int mostrarClubesConMasApariciones(eFigurita arrayFiguritas[], int tamFiguritas, eEquipo equipos[], int tamEquipos, eClub clubs[], int tamClubs)
{
	...
	if (arrayFiguritas != NULL && equipos != NULL && clubs != NULL && tamFiguritas > 0 && tamEquipos > 0 && tamClubs > 0)
	{
		printf("  *** clubes con mas apariciones en el album ***\n");
		printf("---------------------------------\n");

		contadorDeClubs(arrayFiguritas, tamFiguritas, clubs, tamClubs, contadores);

		mayorCantClubs = buscarMayorCantidadDeClubs(contadores, tamClubs);

		for (int i = 0; i < tamClubs; i++)
		{
			if (contadores[i] == mayorCantClubs)
			{
				printf("\nEl club que mas salio es: %s", clubs[i].nombre);
				for(int j = 0; j < 10; j++)
				{
					if(strcmp(nombree[j], " ") == 0)
					{
						strcpy(nombree[j], clubs[i].nombre);
						j = 9;
					}
				}
			}
...
void pegarFigurita(eAlbum *album, int indiceLibre, eFigurita arrayFiguritas[], int indiceRandom)
{
	for (int i = 0; i < 55; i++)
	{
		...
		{
			album->id[indiceLibre] = arrayFiguritas[indiceRandom].id;
			album->isEmpty[indiceLibre] = OCUPADO;
